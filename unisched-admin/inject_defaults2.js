...
const injection = `const DEFAULT_SUBJECTS = [
...
if(!code.includes('const DEFAULT_SUBJECTS = [')) {
  const target = `    ];`;
  
  // Replace the FIRST occurrence of `];` AFTER `const FIXED_SLOT_DEFS = [`
  const fixedSlotIdx = code.indexOf('const FIXED_SLOT_DEFS = [');
  const targetIdx = code.indexOf(target, fixedSlotIdx);
  
  if (targetIdx !== -1) {
     const before = code.slice(0, targetIdx + target.length);
     const after = code.slice(targetIdx + target.length);
     code = before + '\n\n' + injection + after;
